...
function SidebarIcon({ segment }: { segment: string }) {
  ...
  if (segment === "relatorios")
    return <FileSpreadsheet className={iconClass} />;
  return <UserSquare2 className={iconClass} />;
}

function getSidebarSubItems(
  basePath: string,
  segment: string,
): SidebarSubItem[] {
  ...
  if (segment === "comissao") {
    return [
      { label: "Registar", path: `/${basePath}/comissao/registrar` },
      { label: "Visualizar", path: `/${basePath}/comissao/visualizar` },
    ];
  }
  if (segment === "auditoria") {
    return [
      { label: "Registar", path: `/${basePath}/auditoria/registrar` },
      { label: "Visualizar", path: `/${basePath}/auditoria/visualizar` },
    ];
  }
  return [];
}
...
export function BackofficeLayout({
  ...
  const activeSubPath = useMemo(
    () =>
      activeSubItems.find((item) => location.pathname.startsWith(item.path))
        ?.path ??
    ...
    <div className="min-h-screen bg-[#f3f4f6] text-[#111827]">
      ...
      >
        <div className="flex h-full flex-col p-4">
          <div className={`${isSidebarCollapsed ? "px-0 py-2" : "px-1 py-2"}`}>
            ...
            >
              <img
                src="/images/logo.svg"
                alt="SIVO-UP"
                className="h-14 w-14 rounded-full border border-[#d1d5db] bg-white object-contain p-0.5"
              />
              {!isSidebarCollapsed ? (
                <p className="text-ui-2xl font-bold tracking-[-0.02em] text-[#111827]">
                  SIVO-UP
                </p>
              ) : null}
            </div>
          ...
          {!isSidebarCollapsed ? (
            <div className="mt-6">
              <p className="px-2 text-[13px] font-medium text-[#4b5563]">
                Plataforma
              </p>
            </div>
          ) : null}
          ...
          <nav className="mt-3 space-y-1">
            {navItems.map((item) => {
              ...
              const itemTargetPath =
                getSidebarSubItems(basePath, item.segment)[0]?.path ?? itemPath;
          ...
          <div className="mt-auto">
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" side="right" className="w-64">
                ...
                <DropdownMenuSeparator />
                <DropdownMenuItem
                  onSelect={() => navigate(`/${basePath}/perfil`)}
                >
      ...
      <div
        className={`transition-[padding] duration-200 ${isSidebarCollapsed ? "lg:pl-[86px]" : "lg:pl-[286px]"}`}
      >
            ...
            <div className="flex items-center justify-between gap-3">
              <div className="flex min-w-0 items-center gap-3">
                <img
                  src="/images/logo.svg"
                  alt="SIVO-UP"
                  className="h-10 w-10 shrink-0 rounded-full border border-[#d1d5db] bg-white object-contain p-0.5"
                />
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-64">
                  ...
                  {navItems.map((item) => {
                    const itemPath = `/${basePath}/${item.segment}`;
                    const itemTargetPath =
                      getSidebarSubItems(basePath, item.segment)[0]?.path ??
                      itemPath;
                    return (
                      <DropdownMenuItem
                        key={item.segment}
                        onSelect={() => navigate(itemTargetPath)}
                      >
                        <SidebarIcon segment={item.segment} />
                        {item.label}
                      </DropdownMenuItem>
      ...
      {isPasswordModalOpen ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <form onSubmit={changePassword} className="w-full max-w-md">
