  ...
  return (
    <AppShell title={title} actions={<PrimaryButton type="button" onClick={exportLogs}><Download className="mr-2 h-4 w-4" />Exportar Logs (CSV)</PrimaryButton>}>
      ...
      >
        ...
        <div className="mt-5 overflow-hidden rounded-md border border-slate-200 bg-white">
          <table className="w-full text-left text-[14px]">
            ...
            <tbody className="divide-y divide-slate-100 bg-white text-[14px] text-slate-700">
              {pagedLogs.map((log) => (
                <tr key={log.id} className="transition hover:bg-slate-50">
                  ...
                  <td className="px-4 py-4 text-center">
                    <button type="button" onClick={() => setDetailLog(log)} className="inline-flex h-8 w-8 items-center justify-center rounded-md border border-slate-200 text-slate-500 transition hover:bg-slate-50 hover:text-slate-700">
      ...
      <DetailsModal
        ...
        subtitle="Consulta detalhada do evento auditado."
        onClose={() => setDetailLog(null)}
      >
        {detailLog ? (
          <div className="grid gap-4 md:grid-cols-2">
            <div><p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-400">Timestamp</p><p className="mt-1 text-[14px] text-slate-700">{detailLog.timestamp}</p></div>
            <div><p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-400">Utilizador</p><p className="mt-1 text-[14px] text-slate-700">{detailLog.user}</p></div>
            <div><p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-400">Acção</p><p className="mt-1 text-[14px] text-slate-700">{detailLog.action}</p></div>
            <div><p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-400">Eleição</p><p className="mt-1 text-[14px] text-slate-700">{detailLog.election}</p></div>
            <div><p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-400">IP</p><p className="mt-1 text-[14px] text-slate-700">{detailLog.ip}</p></div>
            <div><p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-400">Severidade</p><p className="mt-1 text-[14px] text-slate-700">{detailLog.severity}</p></div>
            <div className="md:col-span-2"><p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-400">Detalhes</p><p className="mt-1 text-[14px] leading-7 text-slate-700">{detailLog.details}</p></div>
          </div>
        ) : null}
      </DetailsModal>
    </AppShell>
